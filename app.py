        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    className="chart-card",
                    children=[
                        html.H3("Postcode Summary for Location = JAC Motors", style={"marginTop": "0", "marginBottom": "12px"}),
                        html.P(
                            "This section only includes rows where the Location column is exactly 'JAC Motors'. It extracts the corresponding postcodes and summarises them by postcode and postcode-derived state.",
                            style={"color": "#666", "marginTop": "0"},
                        ),
...
def jac_motors_state_table(dff):
    """Extract postcodes for rows where Location is exactly JAC Motors and summarise them."""
    output_columns = ["Location", "Postcode", "Derived State", "Lead Count", "Share"]
    ...
    if "Location" not in dff.columns:
        ...
        if jac_df.empty:
            ...
            jac_df["Postcode"] = jac_df.get("Postcode_label", pd.Series("Unknown", index=jac_df.index))
            jac_df["Postcode"] = jac_df["Postcode"].fillna("Unknown").astype("string")
            jac_df["Derived State"] = jac_df["Derived State"].fillna("Unknown").astype("string")

            output = (
                jac_df
                .groupby(["Location", "Postcode", "Derived State"], dropna=False)
                .size()
                .reset_index(name="Lead Count")
                .sort_values(["Lead Count", "Derived State", "Postcode"], ascending=[False, True, True])
            )
