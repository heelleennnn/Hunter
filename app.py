        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    className="chart-card",
                    children=[
                        html.H3("JAC Motors Locations by Postcode-Derived State", style={"marginTop": "0", "marginBottom": "12px"}),
                        html.P(
                            "This section only includes rows where Location contains 'JAC Motors'. State is derived from postcode first, with the State column used only when postcode is missing or invalid.",
                            style={"color": "#666", "marginTop": "0"},
                        ),
...
def jac_motors_state_table(dff):
    """Show postcode-derived state breakdown only for locations containing JAC Motors."""
    if "Location" not in dff.columns:
        output = pd.DataFrame(columns=["Derived State", "Lead Count", "Postcodes", "Share"])
    else:
        jac_df = dff[
            dff["Location"]
            .astype("string")
            .str.contains("JAC Motors", case=False, na=False)
        ].copy()

        if jac_df.empty:
            output = pd.DataFrame(columns=["Derived State", "Lead Count", "Postcodes", "Share"])
        else:
            if "Postcode_label" in jac_df.columns:
                postcodes = (
                    jac_df[jac_df["Postcode_label"].ne("Unknown")]
                    .groupby("Derived State")["Postcode_label"]
                    .apply(lambda x: ", ".join(sorted(x.dropna().astype(str).unique())))
                    .reset_index(name="Postcodes")
                )
            else:
                postcodes = pd.DataFrame(columns=["Derived State", "Postcodes"])

            output = (
                jac_df.groupby("Derived State", dropna=False)
                .size()
                .reset_index(name="Lead Count")
                .sort_values("Lead Count", ascending=False)
            )
            output = output.merge(postcodes, on="Derived State", how="left")
            output["Postcodes"] = output["Postcodes"].fillna("Unknown")
            output["Share"] = (output["Lead Count"] / output["Lead Count"].sum() * 100).round(1).astype(str) + "%"
            output = output[["Derived State", "Lead Count", "Postcodes", "Share"]]
...
def update_dashboard(start_date, end_date, states, forms, order_badges):
    ...
    jac_state_table = jac_motors_state_table(dff)
